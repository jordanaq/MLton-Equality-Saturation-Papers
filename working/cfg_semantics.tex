...
\begin{figure}[H]
  ...
  \begin{mathpar}
    \inferrule*[left=\textsc{Return}]
      ...
      { \langle \texttt{return} (R \, X), s\rangle \Downarrow \bttt{Ok}(v) } \\
    ...
    \inferrule*[left=\textsc{Goto}]
      ...
      { \langle \texttt{goto}_b ( R_1 X_1, \dots, R_n X_n ), s\rangle \Downarrow r }
  
    \inferrule*[left=\textsc{Raise}]
      {
        \llbracket R_1 X_1 \rrbracket_s = v_1 \quad \dots \quad \llbracket R_n X_n \rrbracket_s = v_n \\
      }
      { \langle \texttt{raise } ( R_1 X_1, \dots, R_n X_n), s\rangle \Downarrow \bttt{Err} (v_1, \dots, v_n) }
  
    \inferrule*[left=\textsc{Call-Ok}]
      {
        \llbracket R_i X_i \rrbracket_s = v_i \quad 
        \langle b, \langle x \rangle, \langle \tau \rangle, t \rangle = \bttt{block}(b_{ok}) \\
        \llbracket f (\dots, v_i, \dots) \rrbracket_s = \bttt{Ok}(v : \tau) \quad
        \langle t, s[x \mapsto v ]\rangle \Downarrow r
      }
      { \langle \texttt{call}_f ( \dots, R_i X_i, \dots ) \Rightarrow b_{ok} ; b_{err}, s\rangle \Downarrow r }

    \inferrule*[left=\textsc{Call-Err}]
      {
        \llbracket R_i X_i \rrbracket_s = v_i \quad 
        \langle b, \langle x \rangle, \langle \tau \rangle, t \rangle = \bttt{block}(b_{err}) \\
        \llbracket f (\dots, v_i, \dots) \rrbracket_s = \bttt{Err}(v : \tau) \quad
        \langle t, s[x \mapsto v ]\rangle \Downarrow r
      }
      { \langle \texttt{call}_f ( \dots, R_i X_i, \dots ) \Rightarrow b_{ok} ; b_{err}, s\rangle \Downarrow r }
    ...
    \inferrule*[lab=\textsc{Match-Case}]
      {
        ...
        \langle \texttt{goto}_{b_i} (R_{i,1} X_{i,1}, \dots ), s \rangle \Downarrow r 
      }
      {
        \langle
          \texttt{match} (
    ...
    \inferrule*[lab=\textsc{Match-Default}]
      {
        ...
        \langle \texttt{goto}_{b_d} (R_{d,1} X_{d,1}, \dots ), s \rangle \Downarrow r
      }
      {
        \langle
          \texttt{match} (
